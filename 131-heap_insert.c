...
void array_from_heap(heap_t ***array, heap_t *root, int index, int size);
 ...
 * @array: Double pointer to the array to be created from the Heap
...
void array_from_heap(heap_t ***array, heap_t *root, int index, int size)
{
	if (index >= size)
		return;
	*array[index] = root;
	array_from_heap(array, root->left, 2 * index + 1, size);
	array_from_heap(array, root->right, 2 * index + 2, size);
}

/**
 * create_heap - Creates a Max Binary Heap tree from an array
 *
 * @arr: Pointer to the first element of the array to be added to the heap
